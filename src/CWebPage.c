...
/* test putting it in a clipped window ...
   default was '0'
   Has a positive effect, and adds vert scroll bar!
*/
#define V_OFF   20
#define H_OFF   20
...
HRESULT STDMETHODCALLTYPE Site_GetWindowContext(IOleInPlaceSite FAR* This, LPOLEINPLACEFRAME FAR* lplpFrame, LPOLEINPLACEUIWINDOW FAR* lplpDoc, LPRECT lprcPosRect, LPRECT lprcClipRect, LPOLEINPLACEFRAMEINFO lpFrameInfo)
{
	...
	*lplpDoc = 0;
    ...
    /* Just to test offsetting and clipping the client rectangle */
    if (H_OFF) {
        lprcPosRect->left += H_OFF;
        lprcPosRect->right -= (2 * H_OFF);
        lprcClipRect->left += H_OFF;
        lprcClipRect->right -= (2 * H_OFF);
    }
    if (V_OFF) {
        lprcPosRect->top += V_OFF;
        lprcPosRect->bottom -= (V_OFF * 2);
        lprcClipRect->top += V_OFF;
        lprcClipRect->bottom -= (V_OFF * 2);
    }
...
void UnEmbedBrowserObject(HWND hwnd)
{
    ...
    browserHandle = (IOleObject **)GetWindowLongPtr(hwnd, GWLP_USERDATA);
#else
	...
	if (!OleCreate(&CLSID_WebBrowser, &IID_IOleObject, OLERENDER_DRAW, 0, (IOleClientSite *)_iOleClientSiteEx, &MyIStorage, (void**)&browserObject))
	{
		...
		*((IOleObject **)ptr) = browserObject;
			...
			!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
		{
			...
			webBrowser2->lpVtbl->put_Left(webBrowser2, H_OFF);
			webBrowser2->lpVtbl->put_Top(webBrowser2, V_OFF);
			webBrowser2->lpVtbl->put_Width(webBrowser2, rect.right - (2 * H_OFF));
			webBrowser2->lpVtbl->put_Height(webBrowser2, rect.bottom - (2 * V_OFF));
...
/* just a test string to display */
#define TEST_STRING2 "<H2><CENTER>HTML string test</CENTER></H2><P><FONT COLOR=RED>This is a <U>HTML string</U> in memory.</FONT></P>" \
"<P>And a LINK <a href=\"http://www.microsoft.com\">http://www.microsoft.com</a></p>"

#define TEST_STRING "<!DOCTYPE html><html><head><meta charset=\"utf-8\"><title>test border 2</title>" \
"<style>body{ border: 2px solid green; position: absolute; top : 0; right : 0; bottom : 0; left : 0; overflow: auto; }" \
"</style></head><body>" \
TEST_STRING2 \
"</body></html>"

#ifndef TEST_URL
#define TEST_URL "http://www.microsoft.com"
// #define TEST_URL "C:\\Users\\user\\Downloads\\quickref_5.6.0.html"
#endif
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hInstNULL, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	if (OleInitialize(NULL) == S_OK)
	{
							...
							HWND_DESKTOP, NULL, hInstance, 0)))
		{
			...
			DisplayHTMLStr(msg.hwnd, TEST_STRING );
						...
						HWND_DESKTOP, NULL, hInstance, 0)))
		{
			...
			DisplayHTMLPage(msg.hwnd, TEST_URL);
